...
import type { Coordinates, ClimateDataResponse, OceanMeasurement } from './utils/types';
...
function App() {
  ...
  const handleLocationChange = (coords: Coordinates) => {
    ...
    if (isConnected) {
      // Send message in new real data server format
      sendMessage({
        type: 'getOceanData',
        payload: {
          lat: coords.lat,
      ...
      console.log(`📡 Requesting real ocean data for: ${coords.lat.toFixed(4)}°N, ${coords.lng.toFixed(4)}°E`);
    }
  };
              ...
              <div style={{ 
                ...
                {(() => {
                  ...
                  return Object.entries(groupedData).map(([category, categoryData]) => {
                    const config = categoryConfig[category] || { icon: '📊', color: '#6b7280', title: category.replace('_', ' ') };
