#!/usr/bin/env python3
"""
Historical acidity downloader for CMEMS Global Ocean Biogeochemistry Hindcast (1993-2022).
...
class AcidityHistoricalDownloader(AcidityDownloader):
    """Downloads historical acidity data from CMEMS (1993-2022)."""
    
    def __init__(self, config_path: Optional[Path] = None):
        """Initialize Historical Acidity downloader."""
        # Initialize base downloader with specific dataset name
        from .base_downloader import BaseDataDownloader
        BaseDataDownloader.__init__(self, "acidity_historical", config_path)
        
        # Load acidity_historical specific configuration
        self.dataset_name = "acidity_historical"
        ...
        # Update paths for historical data
        self.raw_data_path = self.raw_data_path.parent / "acidity_historical"
        self.raw_data_path.mkdir(parents=True, exist_ok=True)
        
        # Create processed data directories if needed
        if self.needs_coord_harmonization:
            self.harmonized_path = self.processed_data_path / "unified_coords" / "acidity_historical"
        ...
        self.logger.info(f"Initialized Historical Acidity downloader for CMEMS product: {self.product_id}")
    
    # Import methods from parent AcidityDownloader
    def download_date(self, target_date: date) -> bool:
        """Download historical acidity data for a specific date."""
    ...
    def _get_filename_for_date(self, target_date: date) -> str:
        """Get filename for historical acidity file for given date."""
        return f"acidity_historical_{target_date.strftime('%Y%m%d')}.nc"
    
    def validate_date_range(self, start_date: date, end_date: date) -> tuple[date, date]:
        """
        Validate and adjust date range for historical dataset.
        ...
        """
        # Historical dataset boundaries
        dataset_start = date(1993, 1, 1)
        dataset_end = date(2022, 12, 31)
        
        # Adjust dates to fit within historical dataset bounds
        ...
        if validated_start > dataset_end or validated_end < dataset_start:
            raise ValueError(
                f"Date range {start_date} to {end_date} is outside historical dataset range "
    ...
    def _log_api_data_sample(self, target_date: date, processed_file: Path) -> dict:
        ...
        try:
            ...
            self.logger.info(f"Historical API sample data generated: {sample_file}")
            return api_sample_data
            
        except Exception as e:
            self.logger.error(f"Error generating historical API sample: {e}")
    ...
    def download_date_range(self, start_date: Union[str, date], end_date: Union[str, date], max_files: Optional[int] = None) -> Dict[str, Any]:
        """
        Download historical acidity data for a date range.
